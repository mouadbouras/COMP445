...
.action(function(req,optional){
    ...
    {
        //console.log(url)
        //console.log("1" + host +  " | "+ url + " | "+ verb + " | " + header)        
        client.getUDPRequest(url.host , url.pathname+qs , vrb,header);
    }
    else{      
        var host = (url.pathname.indexOf("/") >= 0) ? url.pathname.substr(0,url.pathname.indexOf("/")) : url.pathname ;
        var path =  (url.pathname.indexOf("/") >= 0) ? url.pathname.substr(url.pathname.indexOf("/")) : "/" ;
        path = (path.trim() == "")? "/" : path; 
        client.getUDPRequest(host , path , vrb,header);    
    }
...
.action(function(req,optional){
  ...
  if(url.host != null && url.pathname != null)
  {
      client.postUDPRequest(url.host , url.pathname+qs , vrb,header,content);
  }
  else{
    var host = (url.pathname.indexOf("/") >= 0) ? url.pathname.substr(0,url.pathname.indexOf("/")) : url.pathname ;
    var path =  (url.pathname.indexOf("/") >= 0) ? url.pathname.substr(url.pathname.indexOf("/")) : "/" ;   
    path = (path.trim() == "")? "/" : path; 
      client.postUDPRequest(host,path,vrb,header,content);    
  }  
